...
class MealRequestPage extends StatefulWidget {
  const MealRequestPage({super.key});

  @override
  State<MealRequestPage> createState() => _MealRequestPageState();
}

class _MealRequestPageState extends State<MealRequestPage> {
  ...
  String _selectedMealType = 'Chọn loại suất ăn';
  ...
  Future<void> _onSubmit() async {
    ...
    if (_selectedMealType == 'Chọn loại suất ăn') {
      AppNotice.showError(context, 'Vui lòng chọn loại suất ăn');
      return;
    }
    if (_mealCountController.text.trim().isEmpty) {
      AppNotice.showError(context, 'Vui lòng nhập số suất ăn');
      return;
    }
    ...
    try {
      ...
      final message = await _requestsService.createRequest(
        ...
        fields: <String, dynamic>{
          ...
          'Loại suất ăn': _selectedMealType,
          'Số suất ăn': _numericText(_mealCountController.text),
          if (_notesController.text.trim().isNotEmpty) 'Ghi chú': _notesController.text.trim(),
        },
      );
      AppLoading.hide();
      if (!mounted) return;
  ...
  Future<void> _pickMealType() async {
    final selected = await _showPicker<String>(
      title: 'Chọn loại suất ăn',
      items: _mealTypes,
      labelOf: (item) => item,
      subtitleOf: (item) => '',
      isSelected: (item) => item == _selectedMealType,
    );
    if (selected != null) {
        ...
        child: ListView(
          ...
          children: [
            ...
            _SelectorCard(
              ...
              onTap: _pickMealType,
              requiredMark: true,
              isPlaceholder: _selectedMealType == 'Chọn loại suất ăn',
            ),
            const SizedBox(height: RequestFormStyle.itemGap),
            _InputCard(
              icon: Icons.confirmation_number_outlined,
              hintText: 'Nhập số suất ăn',
